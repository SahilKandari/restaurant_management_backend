...
func AddOTPtoRedis(otp string, email string, c *gin.Context) error {
	key := otpKeyPrefix + email
	redisClient := config.RedisClient
	// hashing the OTP
	data, _ := bcrypt.GenerateFromPassword([]byte(otp), 6)
...
func VerifyOTP(otp string, email string, c *gin.Context) (error, bool) {
	key := otpKeyPrefix + email
